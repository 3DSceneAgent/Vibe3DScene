...
def test_route_after_initialize_request_always_syncs_reference_catalog_first():
    ...
    assert (
        graph_module._route_after_initialize_request({"router_need_clarification": True})
        == "sync_reference_catalog"
    )


def test_route_after_sync_reference_catalog_always_prepares_reference_context():
    assert graph_module._route_after_sync_reference_catalog({"router_need_clarification": True}) == "prepare_reference_context"
