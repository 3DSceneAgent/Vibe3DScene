#!/bin/bash
# Start API + MCP services for local-client mode by default.
# Behavior is still driven by the final BLENDER_MODE value.
...
DEFAULT_BLENDER_MODE="local-client"
...
usage() {
    cat <<'EOF'
Usage: ./scripts/run_local_client.sh [options]
...
Examples:
  ...
  ./scripts/run_local_client.sh --blender-mode headless
  ./scripts/run_local_client.sh --workers 2
EOF
}
